-- Daily all-time rank snapshots + rank_yesterday on leaderboard_view
...
-- Writes via service role / postgres only (cron); no INSERT for anon JWT
...
-- Refresh PostgREST schema cache so .from('leaderboard_view') resolves immediately
